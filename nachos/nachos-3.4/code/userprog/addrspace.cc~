...
// first, set up the translation 
	...
	pageTable[i].physicalPage = FindAnEmptyPageAndMark();	// Thay pageTable[i].physicalPage = i -> hàm tìm 1 trang trống và đánh dấu đã sử dụng
...
// Bo sung doan code trong file pdf
...
// first, set up the translation
	pageTable = new TranslationEntry[numPages];
	for (i = 0; i < numPages; i++) {pageTable[i].virtualPage = i; // for now, virtual page # = phys page #
		pageTable[i].physicalPage = FindAnEmptyPageAndMark();
		pageTable[i].valid = TRUE;
		pageTable[i].use = FALSE;
		pageTable[i].dirty = FALSE;
		pageTable[i].readOnly = FALSE; // if the code segment was entirely on
...
// Copy initData into the remain part of lastCodePage
		if (firstDataPageSize > 0)
			executable->ReadAt(&(machine->mainMemory[noffH.code.virtualAddr])+(pageTable[i-1].physicalPage*PageSize +
